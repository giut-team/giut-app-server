...
public class CompetitionIngestion {
    ...
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "competition_id")
    ...
    @Column(name = "normalized_input_url", columnDefinition = "text")
    private String normalizedInputUrl;

    @Column(name = "normalized_input_url_hash", length = 64)
    private String normalizedInputUrlHash;

    @Enumerated(EnumType.STRING) @Column(name = "ingestion_status", nullable = false, length = 30)
    private Status ingestionStatus;

    @Enumerated(EnumType.STRING) @Column(name = "extraction_status", nullable = false, length = 20)
    private ExtractionStatus extractionStatus;

    @Enumerated(EnumType.STRING) @Column(name = "review_status", nullable = false, length = 20)
    private ReviewStatus reviewStatus;

    @JdbcTypeCode(SqlTypes.JSON) @Column(name = "missing_fields", columnDefinition = "json")
