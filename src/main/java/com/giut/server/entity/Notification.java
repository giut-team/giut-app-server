...
public class Notification {
    ...
    @Enumerated(EnumType.STRING) @Column(name = "notification_type", nullable = false, length = 40) private Type notificationType;

    @Column(nullable = false, length = 120) private String title;

    @Column(nullable = false, columnDefinition = "text") private String body;

    @Column(name = "action_path", columnDefinition = "text") private String actionPath;

    @JdbcTypeCode(SqlTypes.JSON) @Column(columnDefinition = "json") private String payload;

    @Column(name = "dedup_key", length = 200) private String dedupKey;
