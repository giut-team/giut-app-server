...
public class DuplicateAuditCandidate {
    ...
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "competition_id_a", nullable = false)
    private Competition competitionA;

    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "competition_id_b", nullable = false)
    private Competition competitionB;

    @Column(name = "match_score", precision = 5, scale = 4)
    private BigDecimal matchScore;

    @JdbcTypeCode(SqlTypes.JSON) @Column(name = "match_basis", columnDefinition = "json")
    private String matchBasis;

    @Enumerated(EnumType.STRING) @Column(name = "ai_judgement", length = 20)
    private AiJudgement aiJudgement;

    @Enumerated(EnumType.STRING) @Column(name = "admin_decision", nullable = false, length = 20)
